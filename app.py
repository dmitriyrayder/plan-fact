...
def prepare_data(df_fact, df_plan):
    ...
    df_merged['Revenue_Diff_Pct'] = (safe_divide(df_merged['Revenue_Diff'], df_merged['Revenue_Plan']) * 100).round(2)

    df_merged['Units_Diff'] = df_merged['Units_Fact'] - df_merged['Units_Plan']
    df_merged['Units_Diff_Pct'] = (safe_divide(df_merged['Units_Diff'], df_merged['Units_Plan']) * 100).round(2)
...
def safe_divide(numerator, denominator, default=0):
    ...
    if isinstance(numerator, pd.Series) and isinstance(denominator, pd.Series):
        return numerator.div(denominator).replace([np.inf, -np.inf], default).fillna(default)
...
def calculate_financial_metrics(df_merged, df_fact_detailed):
    ...
    # Отклонения
    metrics['revenue_variance'] = metrics['total_revenue_fact'] - metrics['total_revenue_plan']
    metrics['revenue_variance_pct'] = safe_divide(metrics['revenue_variance'], metrics['total_revenue_plan'], 0) * 100
    metrics['units_variance'] = metrics['total_units_fact'] - metrics['total_units_plan']
    metrics['units_variance_pct'] = safe_divide(metrics['units_variance'], metrics['total_units_plan'], 0) * 100

    # Средний чек
    metrics['avg_check_plan'] = safe_divide(metrics['total_revenue_plan'], metrics['total_units_plan'], 0)
    metrics['avg_check_fact'] = safe_divide(metrics['total_revenue_fact'], metrics['total_units_fact'], 0)
    metrics['avg_check_diff'] = metrics['avg_check_fact'] - metrics['avg_check_plan']
    metrics['avg_check_diff_pct'] = safe_divide(metrics['avg_check_diff'], metrics['avg_check_plan'], 0) * 100

    # ROI и выполнение плана
    metrics['plan_achievement'] = safe_divide(metrics['total_revenue_fact'], metrics['total_revenue_plan'], 0) * 100
    ...
    metrics['store_success_rate'] = safe_divide(metrics['stores_above_plan'], metrics['total_stores'], 0) * 100
...
def perform_abc_analysis(df_merged):
    ...
    store_revenue['Revenue_Cumsum_Pct'] = safe_divide(store_revenue['Revenue_Cumsum'], total_revenue, 0) * 100
    ...
    store_revenue['ABC_Category'] = store_revenue['Revenue_Cumsum_Pct'].apply(assign_abc_category)
    store_revenue['Performance'] = safe_divide(store_revenue['Revenue_Fact'], store_revenue['Revenue_Plan'], 0) * 100
...
# Функция для отображения статистики загрузки
def show_data_load_statistics(df_fact, df_plan):
    """Отображение статистики загрузки данных"""
    st.success("✅ Данные успешно загружены!")

    col1, col2 = st.columns(2)

    with col1:
        st.markdown("### 📋 Таблица ФАКТ")
        st.metric("Записей загружено", f"{len(df_fact):,}".replace(',', ' '))
        st.metric("Уникальных магазинов", df_fact['Magazin'].nunique() if 'Magazin' in df_fact.columns else 0)
        st.metric("Период данных", f"{df_fact['Datasales'].min().strftime('%d.%m.%Y')} - {df_fact['Datasales'].max().strftime('%d.%m.%Y')}" if 'Datasales' in df_fact.columns else "N/A")

    with col2:
        st.markdown("### 📊 Таблица ПЛАН")
        st.metric("Записей загружено", f"{len(df_plan):,}".replace(',', ' '))
        st.metric("Уникальных магазинов", df_plan['Magazin'].nunique() if 'Magazin' in df_plan.columns else 0)
        st.metric("Периодов плана", df_plan['Month'].nunique() if 'Month' in df_plan.columns else 0)
...
def main():
    ...
    with st.expander("📚 Требования к структуре данных", expanded=False):
        col1, col2 = st.columns(2)

        with col1:
            st.markdown("### 📋 Таблица ФАКТ (Факт продаж)")
            st.markdown("""
            **Обязательные колонки:**
            - `Magazin` - название магазина (текст)
            - `Datasales` - дата продажи (формат: ДД.ММ.ГГГГ, например: 15.01.2025)
            - `Segment` - сегмент товара (Premium, Medium, Economy, Sun)
            - `Price` - цена товара (число)
            - `Qty` - количество (число)
            - `Sum` - сумма продажи (число)
            ...
            **Пример строки:**
            | Magazin | Datasales | Segment | Price | Qty | Sum |
            |---------|-----------|---------|-------|-----|-----|
            | Салон_1 | 15.01.2025 | Premium | 15000 | 1 | 15000 |
            """)

        with col2:
            st.markdown("### 📊 Таблица ПЛАН (План продаж)")
            st.markdown("""
            **Обязательные колонки:**
            - `Magazin` - название магазина (текст)
            - `Segment` - сегмент товара (Premium, Medium, Economy, Sun)
            - `Month` - месяц плана (формат: ГГГГ-ММ, например: 2025-01)
            - `Revenue_Plan` - плановая выручка (число)
            - `Units_Plan` - плановое количество единиц (число)

            **Пример строки:**
            | Magazin | Segment | Month | Revenue_Plan | Units_Plan |
            |---------|---------|-------|--------------|------------|
            | Салон_1 | Premium | 2025-01 | 800000 | 120 |

            **Важно:**
            - Названия магазинов должны совпадать в обеих таблицах
            - Сегменты должны совпадать в обеих таблицах
            - Формат даты строго: ДД.ММ.ГГГГ
            """)

        st.info("💡 **Совет:** Убедитесь, что таблицы публично доступны для чтения (если используете Google Sheets)")
    ...
    if use_demo:
        ...
        show_data_load_statistics(df_fact, df_plan)
    else:
        ...
        if st.sidebar.button("🔄 Загрузить данные", use_container_width=True):
            with st.spinner("Загрузка из Google Sheets..."):
                ...
                if df_fact is not None and df_plan is not None:
                    show_data_load_statistics(df_fact, df_plan)
        else:
    ...
    financial_metrics = calculate_financial_metrics(df_filtered, df_fact_detailed)
    ...
    with tab0:
        ...
        with col2:
            ...
            if financial_metrics['revenue_variance_pct'] > 5:
                st.success(f"✅ План перевыполнен на {financial_metrics['revenue_variance_pct']:.1f}% (+{format_number(financial_metrics['revenue_variance'])} ₴)")
            elif financial_metrics['revenue_variance_pct'] >= -5:
                st.info(f"ℹ️ План выполнен на {financial_metrics['plan_achievement']:.1f}%")
            else:
                st.warning(f"⚠️ Недовыполнение плана на {abs(financial_metrics['revenue_variance_pct']):.1f}% ({format_number(financial_metrics['revenue_variance'])} ₴)")
        ...
        segment_table['Revenue_Plan'] = segment_table['Revenue_Plan'].apply(lambda x: f"{format_number(x)} ₴")
        segment_table['Revenue_Fact'] = segment_table['Revenue_Fact'].apply(lambda x: f"{format_number(x)} ₴")
        segment_table['Units_Plan'] = segment_table['Units_Plan'].apply(lambda x: f"{format_number(x)} шт")
        segment_table['Units_Fact'] = segment_table['Units_Fact'].apply(lambda x: f"{format_number(x)} шт")
        segment_table['Achievement_%'] = segment_table['Achievement_%'].apply(lambda x: f"{x:.1f}%")
        segment_table['Avg_Check'] = segment_table['Avg_Check'].apply(lambda x: f"{format_number(x)} ₴")
    ...
    with tab1:
        ...
        with col2:
            ...
            perf_by_segment['Performance'] = (
                safe_divide(perf_by_segment['Revenue_Fact'], perf_by_segment['Revenue_Plan']) * 100
            ).round(1)
    ...
    with tab2:
        ...
        store_summary['Revenue_Diff_Pct'] = (
            safe_divide(store_summary['Revenue_Diff'], store_summary['Revenue_Plan']) * 100
        ).round(2)

        store_summary['Units_Diff'] = store_summary['Units_Fact'] - store_summary['Units_Plan']
        store_summary['Units_Diff_Pct'] = (
            safe_divide(store_summary['Units_Diff'], store_summary['Units_Plan']) * 100
        ).round(2)
    ...
    with tab5:
        ...
        with col1:
            ...
            fig_abc_pie = px.pie(
                ...
                color='Category',
                color_discrete_map={'A': '#51cf66', 'B': '#ffd43b', 'C': '#ff6b6b'}
            )
        ...
        with col2:
            ...
            # Парето кривая
            abc_pareto = abc_analysis[['Magazin', 'Revenue_Fact', 'Revenue_Cumsum_Pct', 'ABC_Category']].head(20)
            ...
            fig_pareto.update_layout(
                ...
                yaxis2=dict(title='Накопительный %', overlaying='y', side='right'),
                hovermode='x unified',
                height=350,
                legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1)
            )
        ...
        abc_filtered = abc_analysis[abc_analysis['ABC_Category'].isin(selected_abc)].copy()

        # Форматирование таблицы
        abc_display = abc_filtered[['Magazin', 'ABC_Category', 'Revenue_Plan', 'Revenue_Fact', 'Performance', 'Revenue_Cumsum_Pct']].copy()
        abc_display.columns = ['Магазин', 'ABC', 'План ₴', 'Факт ₴', 'Выполнение %', 'Накопительно %']
        ...
        with col2:
            ...
            csv_summary = summary_data.to_csv(index=False, encoding='utf-8-sig')
            st.download_button(
